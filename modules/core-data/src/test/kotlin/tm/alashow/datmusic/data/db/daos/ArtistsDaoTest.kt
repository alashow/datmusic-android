...
@HiltAndroidTest
class ArtistsDaoTest : BaseTest() {

    @Inject
    lateinit var database: AppDatabase

    @Inject
    lateinit var dao: ArtistsDao

    private val testItems = (1..5).map { SampleData.artist() }
    ...
    fun count_withParams() = runTest {
        val paramlessItems = (1..5).map { SampleData.artist() }
        val items = testItems.map { it.copy(params = testParams.toString()) }
